...
// -----------------------------------------
//
// Source: StartCenterBuyerPOTabLT1.java
// Date  : 07 Nov 2019 19:16:08 PST
 ...
 * Automatically generated load test program.
 */
public class StartCenterBuyerPOTabLT1 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "StartCenterBuyerPOTabLT1ExtParams.dat";// input file name for very large parameter values
	...
	private String C_PcompID15 = null;                   // var declaration from web admin var handler: scope = per loop
	private String C_PcompID14 = null;                   // var declaration from web admin var handler: scope = per loop
	private String C_PcompID13 = null;                   // var declaration from web admin var handler: scope = per loop
	private String C_PcompID12 = null;                   // var declaration from web admin var handler: scope = per loop
	private String C_PcompID11 = null;                   // var declaration from web admin var handler: scope = per loop
	private String C_PcompID10 = null;                   // var declaration from web admin var handler: scope = per loop
	...
	private String C_Pcomp16 = null;                     // var declaration from web admin var handler: scope = per loop
	...
	private String C_PresentationID = null;              // var declaration from web admin var handler: scope = per loop
	
	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public StartCenterBuyerPOTabLT1()
	{
		super();
	}
	 ...
	 */
	public StartCenterBuyerPOTabLT1(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("<<< C_PcompID15 = " + C_PcompID15);
		log("<<< C_PcompID14 = " + C_PcompID14);
		log("<<< C_PcompID13 = " + C_PcompID13);
		log("<<< C_PcompID12 = " + C_PcompID12);
		log("<<< C_PcompID11 = " + C_PcompID11);
		log("<<< C_PcompID10 = " + C_PcompID10);
		log("<<< C_CsrfToken2 = " + C_CsrfToken2);
		log("<<< C_CsrfToken1 = " + C_CsrfToken1);
		log("<<< C_LoginTimeStamp = " + C_LoginTimeStamp);
		log("<<< C_Pcomp16 = " + C_Pcomp16);
		log("<<< C_Focus2 = " + C_Focus2);
		log("<<< C_Focus1 = " + C_Focus1);
		log("<<< C_PresentationID = " + C_PresentationID);
		...
		log();
		log();
		log("# Page #3: BuyerPO Tab");
		log("# --------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: BuyerPO Tab", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestContent0013 = 
				...
				"&login=jsp" +
				"&localStorage=true" +
				"&loginstamp=" + C_LoginTimeStamp +		// recorded value = 1573178932086
		...
		requestContent0013 = Lib.replaceTextPattern(requestContent0013, "1573178932086", C_LoginTimeStamp, 0);		// search and replace content fragment "1573178932086" with variable 'C_LoginTimeStamp' on all occurrences
		...
		BoundaryBasedExtractorItem boundaryBasedHeaderExtractorItem0013_1 = new BoundaryBasedExtractorItem( "uisessionid=" , "&",1,false,false,0,false);
		...
		String requestFile0014 = "/maximo_mif/ui/login" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&event=loadapp" +
				"&value=user" +
				"&uisessionid=" + C_UISessionID +		// recorded value = 192
				"&_tt=8u3uo24dh0r4275mc4la36eb4h";
		requestFile0014 = Lib.replaceTextPattern(requestFile0014, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 22276
		// content test algorithm: search text
		...
		BoundaryBasedExtractorItem boundaryBasedContentExtractorItem0014_1 = new BoundaryBasedExtractorItem( "csrftoken:'" , "'});",1,false,false,0,false);
		boundaryBasedExtractor.addBoundaryBasedExtractorItem(boundaryBasedContentExtractorItem0014_1);
		...
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0014_8 = new RegExpBasedExtractorItem( "<li role=\"presentation\" id=\"(.*?)-sct_" ,1,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0014_8);

		// setup var extractor for 'C_Focus1'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0014_9 = new RegExpBasedExtractorItem( "button ctype=\"pushbutton\"   type=\"button\"    control=\"true\" id=\"(.*?)\"  class=\"    text pb default\"  style=\"\">OK" ,1,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0014_9);

		// setup var extractor for 'C_PresentationID'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0014_10 = new RegExpBasedExtractorItem( "<li role=\"presentation\" id=\"(.*?)-sct_(.*?)\" ctype=\"tab\">" ,2,2,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0014_10);
		...
		try
		{
			...
			C_CsrfToken1 = boundaryBasedContentExtractorItem0014_1.getResultText();
			log("<<< C_CsrfToken1 = " + C_CsrfToken1);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
		...
		try
		{
			// extract var 'C_PresentationID'
			C_PresentationID = regExpBasedContentExtractorItem0014_10.getResultText();
			log("<<< C_PresentationID = " + C_PresentationID);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PresentationID!= null && C_PresentationID.trim().length() == 0)
			{
				C_PresentationID = "";
				log("runtime note: the extraction of a new value for the variable 'C_PresentationID' ends up with an empty string - a substitute value was set");
				log("<<< C_PresentationID = " + C_PresentationID);
			}
		}
		catch (Exception e) { C_PresentationID = null; log(e); }
		if (C_PresentationID == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PresentationID' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		testURL.replaceRequestHeaderFieldTextPattern("4152", C_UISessionID);		// search within all request header fields a text pattern and replace it with a variable value
		if (userNextProxyConfig != null)
			testURL.setProxy(userNextProxyConfig);
		testURL.setDNSContext(userDNSContext);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		...
		String requestFile0024 = "/maximo_mif/webclient/javascript/tpae-20191028-2106/navsection.js";
		...
		String requestFile0025 = "/maximo_mif/webclient/javascript/tpae-20191028-2106/prototype.js";
		...
		String requestFile0030 = "/maximo_mif/webclient/javascript/tpae-20191028-2106/sessiontimer.js";
		...
		String requestFile0031 = "/maximo_mif/webclient/javascript/tpae-20191028-2106/designer.js";
	...
	boolean executePage_2_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0041 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/ac_ban_divider.png";
		...
		String requestFile0050 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/btn_updatestartcenter.gif";
	...
	boolean executePage_2_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0068 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/layers/mbs/popuplayer.js";
		...
		String requestFile0069 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/menus/menusub.gif";
		...
		String requestFile0070 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/working.gif";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/GIF", null, null);		// Test [70] <- Index 70
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0071 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/edited.png";
		...
		String requestFile0072 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/error.png";
	...
	boolean executePage_2_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0073 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/warning.png";
		...
		String requestFile0074 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/question.png";
		...
		String requestFile0075 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/async/smartfill.png";
	...
	boolean executePage_2_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0109 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/maximo-icon.png" +
				"?v2";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [109] <- Index 109
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0111 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957196";
		...
		requestFile0111 = Lib.replaceTextPattern(requestFile0111, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		String requestFile0112 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957197";
		...
		requestFile0112 = Lib.replaceTextPattern(requestFile0112, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
	...
	boolean executePage_2_5(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0113 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957199";
		...
		requestFile0113 = Lib.replaceTextPattern(requestFile0113, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		String requestFile0114 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957200";
		...
		requestFile0114 = Lib.replaceTextPattern(requestFile0114, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, null);		// Test [114] <- Index 114
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0115 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957202";
		...
		requestFile0115 = Lib.replaceTextPattern(requestFile0115, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		requestFile0116 = Lib.replaceTextPattern(requestFile0116, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		String requestFile0117 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				...
				"&dojo.preventCache=1573178957204";
		...
		requestFile0117 = Lib.replaceTextPattern(requestFile0117, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		String requestFile0121 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/portlet_pageseperator.gif";
		...
		String requestFile0122 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/btn_nextpage.gif";
		...
		String requestFile0123 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/appimg_po.gif";
		...
		String requestFile0124 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/appimg_contpurch.gif";
		...
		String requestFile0125 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/appimg_rfq.gif";
		...
		String requestFile0126 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/btn_editportlet.gif";
		...
		String requestContent0128 = 
				"uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&csrftoken=" + C_CsrfToken1 +		// recorded value = q2a01pghl1hdb52vs6et8knd7b
				"&currentfocus=" + C_Focus1 +		// recorded value = m88dbf6ce-pb
				"&scrollleftpos=0" +
				"&localStorage=true" +
				"&scrolltoppos=0" +
				"&requesttype=SYNC" +
				"&responsetype=text%2Fxml" +
				"&events=%5B%7B%22type%22%3A%22click%22%2C%22targetId%22%3A%22m88dbf6ce-pb%22%2C%22value%22%3A%22%22%2C%22requestType%22%3A%22SYNC%22%2C%22csrftokenholder%22%3A%22q2a01pghl1hdb52vs6et8knd7b%22%7D%5D";
		requestContent0128 = Lib.replaceTextPattern(requestContent0128, "4152", C_UISessionID, 0);		// search and replace content fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestContent0128 = Lib.replaceTextPattern(requestContent0128, "q2a01pghl1hdb52vs6et8knd7b", C_CsrfToken1, 0);		// search and replace content fragment "q2a01pghl1hdb52vs6et8knd7b" with variable 'C_CsrfToken1' on all occurrences
		requestContent0128 = Lib.replaceTextPattern(requestContent0128, "m88dbf6ce-pb", C_Focus1, 0);		// search and replace content fragment "m88dbf6ce-pb" with variable 'C_Focus1' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 1263
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "Quick Insert");		// Test [128] <- Index 128
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestContent0130 = 
				...
				"&requesttype=SYNC" +
				"&responsetype=text%2Fxml" +
				"&events=%5B%7B%22type%22%3A%22changesc%22%2C%22targetId%22%3A%22m1e20cba1-sct%22%2C%22value%22%3A%22672%22%2C%22requestType%22%3A%22SYNC%22%2C%22csrftokenholder%22%3A%22q2a01pghl1hdb52vs6et8knd7b%22%7D%5D";
		requestContent0130 = Lib.replaceTextPattern(requestContent0130, "4152", C_UISessionID, 0);		// search and replace content fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestContent0130 = Lib.replaceTextPattern(requestContent0130, "q2a01pghl1hdb52vs6et8knd7b", C_CsrfToken1, 0);		// search and replace content fragment "q2a01pghl1hdb52vs6et8knd7b" with variable 'C_CsrfToken1' on all occurrences
		requestContent0130 = Lib.replaceTextPattern(requestContent0130, "m1e20cba1", C_Focus2, 0);		// search and replace content fragment "m1e20cba1" with variable 'C_Focus2' on all occurrences
		requestContent0130 = Lib.replaceTextPattern(requestContent0130, "m88dbf6ce-pb", C_Focus1, 0);		// search and replace content fragment "m88dbf6ce-pb" with variable 'C_Focus1' on all occurrences
		requestContent0130 = Lib.replaceTextPattern(requestContent0130, "672", C_PresentationID, 0);		// search and replace content fragment "672" with variable 'C_PresentationID' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 248
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "![CDATA[https://teck.maximo.com/ma");		// Test [130] <- Index 130
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0131 = "/maximo_mif/ui/" +
				"?event=loadapp" +
				"&value=startcntr" +
				"&uniqueid=" + C_PresentationID +		// recorded value = 672
				"&uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&_tt=" + C_tt1;		// recorded value = fbmcasdd7eo5cjv43dodp0473o
		requestFile0131 = Lib.replaceTextPattern(requestFile0131, "fbmcasdd7eo5cjv43dodp0473o", C_tt1, 0);		// search and replace request file fragment "fbmcasdd7eo5cjv43dodp0473o" with variable 'C_tt1' on all occurrences
		requestFile0131 = Lib.replaceTextPattern(requestFile0131, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0131 = Lib.replaceTextPattern(requestFile0131, "672", C_PresentationID, 0);		// search and replace request file fragment "672" with variable 'C_PresentationID' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 19479
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Emergency Approved Purchase Requisition");		// Test [131] <- Index 131
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		BoundaryBasedExtractorItem boundaryBasedContentExtractorItem0131_1 = new BoundaryBasedExtractorItem( "csrftoken:'" , "'});",1,false,false,0,false);
		boundaryBasedExtractor.addBoundaryBasedExtractorItem(boundaryBasedContentExtractorItem0131_1);
		...
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_1 = new RegExpBasedExtractorItem( "td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,1,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_1);

		// setup var extractor for 'C_PcompID8'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_2 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hd_right\" nowrap >" ,1,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_2);

		// setup var extractor for 'C_PcompID15'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_3 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hd_right\" nowrap >" ,2,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_3);

		// setup var extractor for 'C_PcompID14'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_4 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,6,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_4);

		// setup var extractor for 'C_PcompID13'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_5 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,5,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_5);

		// setup var extractor for 'C_PcompID12'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_6 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,4,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_6);

		// setup var extractor for 'C_PcompID11'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_7 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,3,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_7);

		// setup var extractor for 'C_PcompID10'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_8 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hb_right\" nowrap >" ,2,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_8);

		// setup var extractor for 'C_Pcomp16'
		RegExpBasedExtractorItem regExpBasedContentExtractorItem0131_9 = new RegExpBasedExtractorItem( "<td style=\"padding-left:0px;\" class=\"phbr\" id=\"(.*?)-hd_right\" nowrap >" ,3,1,false,false,0,false);
		regExpBasedExtractor.addRegExpBasedExtractorItem(regExpBasedContentExtractorItem0131_9);
		...
		try
		{
			...
			C_PcompID9 = regExpBasedContentExtractorItem0131_1.getResultText();
			log("<<< C_PcompID9 = " + C_PcompID9);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
		...
		try
		{
			...
			C_PcompID8 = regExpBasedContentExtractorItem0131_2.getResultText();
			log("<<< C_PcompID8 = " + C_PcompID8);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
		...
		try
		{
			// extract var 'C_PcompID15'
			C_PcompID15 = regExpBasedContentExtractorItem0131_3.getResultText();
			log("<<< C_PcompID15 = " + C_PcompID15);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID15!= null && C_PcompID15.trim().length() == 0)
			{
				C_PcompID15 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID15' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID15 = " + C_PcompID15);
			}
		}
		catch (Exception e) { C_PcompID15 = null; log(e); }
		if (C_PcompID15 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID15' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'C_PcompID14'
			C_PcompID14 = regExpBasedContentExtractorItem0131_4.getResultText();
			log("<<< C_PcompID14 = " + C_PcompID14);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID14!= null && C_PcompID14.trim().length() == 0)
			{
				C_PcompID14 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID14' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID14 = " + C_PcompID14);
			}
		}
		catch (Exception e) { C_PcompID14 = null; log(e); }
		if (C_PcompID14 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID14' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'C_PcompID13'
			C_PcompID13 = regExpBasedContentExtractorItem0131_5.getResultText();
			log("<<< C_PcompID13 = " + C_PcompID13);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID13!= null && C_PcompID13.trim().length() == 0)
			{
				C_PcompID13 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID13' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID13 = " + C_PcompID13);
			}
		}
		catch (Exception e) { C_PcompID13 = null; log(e); }
		if (C_PcompID13 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID13' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'C_PcompID12'
			C_PcompID12 = regExpBasedContentExtractorItem0131_6.getResultText();
			log("<<< C_PcompID12 = " + C_PcompID12);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID12!= null && C_PcompID12.trim().length() == 0)
			{
				C_PcompID12 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID12' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID12 = " + C_PcompID12);
			}
		}
		catch (Exception e) { C_PcompID12 = null; log(e); }
		if (C_PcompID12 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID12' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'C_PcompID11'
			C_PcompID11 = regExpBasedContentExtractorItem0131_7.getResultText();
			log("<<< C_PcompID11 = " + C_PcompID11);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID11!= null && C_PcompID11.trim().length() == 0)
			{
				C_PcompID11 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID11' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID11 = " + C_PcompID11);
			}
		}
		catch (Exception e) { C_PcompID11 = null; log(e); }
		if (C_PcompID11 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID11' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'C_PcompID10'
			C_PcompID10 = regExpBasedContentExtractorItem0131_8.getResultText();
			log("<<< C_PcompID10 = " + C_PcompID10);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_PcompID10!= null && C_PcompID10.trim().length() == 0)
			{
				C_PcompID10 = "";
				log("runtime note: the extraction of a new value for the variable 'C_PcompID10' ends up with an empty string - a substitute value was set");
				log("<<< C_PcompID10 = " + C_PcompID10);
			}
		}
		catch (Exception e) { C_PcompID10 = null; log(e); }
		if (C_PcompID10 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_PcompID10' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			...
			C_CsrfToken2 = boundaryBasedContentExtractorItem0131_1.getResultText();
			log("<<< C_CsrfToken2 = " + C_CsrfToken2);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
		...
		try
		{
			// extract var 'C_Pcomp16'
			C_Pcomp16 = regExpBasedContentExtractorItem0131_9.getResultText();
			log("<<< C_Pcomp16 = " + C_Pcomp16);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && C_Pcomp16!= null && C_Pcomp16.trim().length() == 0)
			{
				C_Pcomp16 = "";
				log("runtime note: the extraction of a new value for the variable 'C_Pcomp16' ends up with an empty string - a substitute value was set");
				log("<<< C_Pcomp16 = " + C_Pcomp16);
			}
		}
		catch (Exception e) { C_Pcomp16 = null; log(e); }
		if (C_Pcomp16 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'C_Pcomp16' from regexp based pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestFile0132 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/tablebtn_download.gif";
		...
		testURL.replaceRequestHeaderFieldTextPattern("fbmcasdd7eo5cjv43dodp0473o", C_tt1);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("4152", C_UISessionID);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("672", C_PresentationID);		// search within all request header fields a text pattern and replace it with a variable value
		if (userNextProxyConfig != null)
			testURL.setProxy(userNextProxyConfig);
		testURL.setDNSContext(userDNSContext);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		...
		String requestFile0133 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=mc02da76c-content" +
				"&dojo.preventCache=1573178976701";
		requestFile0133 = Lib.replaceTextPattern(requestFile0133, "mc02da76c", C_PcompID8, 0);		// search and replace request file fragment "mc02da76c" with variable 'C_PcompID8' on all occurrences
		requestFile0133 = Lib.replaceTextPattern(requestFile0133, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 717
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "New Purchase Order");		// Test [133] <- Index 133
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0134 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m38bdc6a7-content" +
				"&dojo.preventCache=1573178976702";
		requestFile0134 = Lib.replaceTextPattern(requestFile0134, "m38bdc6a7", C_PcompID9, 0);		// search and replace request file fragment "m38bdc6a7" with variable 'C_PcompID9' on all occurrences
		requestFile0134 = Lib.replaceTextPattern(requestFile0134, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 713
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "Request for Quotations (TCK)");		// Test [134] <- Index 134
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0135 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m5d5ced-content" +
				"&dojo.preventCache=1573178976703";
		requestFile0135 = Lib.replaceTextPattern(requestFile0135, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0135 = Lib.replaceTextPattern(requestFile0135, "m5d5ced", C_PcompID10, 0);		// search and replace request file fragment "m5d5ced" with variable 'C_PcompID10' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 1702
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "Set Chart Options");		// Test [135] <- Index 135
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0136 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=mc213aa1b-content" +
				"&dojo.preventCache=1573178976705";
		requestFile0136 = Lib.replaceTextPattern(requestFile0136, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0136 = Lib.replaceTextPattern(requestFile0136, "mc213aa1b", C_PcompID11, 0);		// search and replace request file fragment "mc213aa1b" with variable 'C_PcompID11' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 1719
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "Requested Date");		// Test [136] <- Index 136
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0137 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=me7e571ea-content" +
				"&dojo.preventCache=1573178976705";
		requestFile0137 = Lib.replaceTextPattern(requestFile0137, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0137 = Lib.replaceTextPattern(requestFile0137, "me7e571ea", C_PcompID12, 0);		// search and replace request file fragment "me7e571ea" with variable 'C_PcompID12' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 2723
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, null);		// Test [137] <- Index 137
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0138 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m61b3509d-content" +
				"&dojo.preventCache=1573178976706";
		requestFile0138 = Lib.replaceTextPattern(requestFile0138, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0138 = Lib.replaceTextPattern(requestFile0138, "m61b3509d", C_PcompID13, 0);		// search and replace request file fragment "m61b3509d" with variable 'C_PcompID13' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 436
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, null);		// Test [138] <- Index 138
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0139 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m98b604fc-content" +
				"&dojo.preventCache=1573178976707";
		requestFile0139 = Lib.replaceTextPattern(requestFile0139, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0139 = Lib.replaceTextPattern(requestFile0139, "m98b604fc", C_PcompID14, 0);		// search and replace request file fragment "m98b604fc" with variable 'C_PcompID14' on all occurrences
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "Services Contract for Contract Admin Test");		// Test [139] <- Index 139
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0140 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m791a2d9c-content" +
				"&dojo.preventCache=1573178976709";
		requestFile0140 = Lib.replaceTextPattern(requestFile0140, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0140 = Lib.replaceTextPattern(requestFile0140, "m791a2d9c", C_PcompID15, 0);		// search and replace request file fragment "m791a2d9c" with variable 'C_PcompID15' on all occurrences
		...
		String requestFile0141 = "/maximo_mif/webclient/components/portletrenderer.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&pcompid=m650f4d66-content" +
				"&dojo.preventCache=1573178976710";
		requestFile0141 = Lib.replaceTextPattern(requestFile0141, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestFile0141 = Lib.replaceTextPattern(requestFile0141, "m650f4d66", C_Pcomp16, 0);		// search and replace request file fragment "m650f4d66" with variable 'C_Pcomp16' on all occurrences
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, null);		// Test [141] <- Index 141
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0142 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/action_insert_plusgpo.gif";
		...
		String requestFile0143 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/appimg_pr.gif";
		...
		String requestFile0144 = "/maximo_mif/webclient/skins/skins-20191028-2106/iot18/images/chartlegend_frame.gif";
		...
		String requestFile0145 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/ibm/tivoli/mbs/dojox/charting/Chart.js";
		...
		String requestFile0146 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/Chart.js";
		...
		String requestFile0147 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/Element.js";
		...
		String requestFile0148 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx.js";
		...
		String requestFile0149 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/renderer.js";
	...
	boolean executePage_3_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0150 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/_base.js";
		...
		String requestFile0151 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/svg.js";
		...
		String requestFile0152 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/shape.js";
		...
		String requestFile0153 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/matrix.js";
		...
		String requestFile0154 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/path.js";
		...
		String requestFile0155 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/SimpleTheme.js";
		...
		String requestFile0156 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/utils.js";
		...
		String requestFile0157 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/gradutils.js";
		...
		String requestFile0158 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/Series.js";
		...
		String requestFile0159 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/axis2d/common.js";
		...
		String requestFile0160 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional.js";
		...
		String requestFile0161 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/lambda.js";
		...
		String requestFile0162 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/array.js";
		...
		String requestFile0163 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/object.js";
		...
		String requestFile0164 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/fold.js";
		...
		String requestFile0165 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/reversed.js";
		...
		String requestFile0166 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/action2d/Tooltip.js";
		...
		String requestFile0167 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/action2d/PlotAction.js";
		...
		String requestFile0168 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/action2d/Base.js";
		...
		String requestFile0169 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojo/fx/easing.js";
	...
	boolean executePage_3_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0170 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/lang/functional/scan.js";
		...
		String requestFile0171 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/action2d/MoveSlice.js";
		...
		String requestFile0172 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/gfx/fx.js";
		...
		String requestFile0173 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/axis2d/Default.js";
		...
		String requestFile0174 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/axis2d/Invisible.js";
		...
		String requestFile0175 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/axis2d/Base.js";
		...
		String requestFile0176 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/scaler/linear.js";
		...
		String requestFile0177 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/scaler/common.js";
		...
		String requestFile0178 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/Pie.js";
		...
		String requestFile0179 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/Base.js";
		...
		String requestFile0180 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/common.js";
		...
		String requestFile0181 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/_PlotEvents.js";
		...
		String requestFile0182 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/Columns.js";
		...
		String requestFile0183 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/plot2d/CartesianBase.js";
		...
		String requestFile0184 = "/maximo_mif/webclient/javascript/dojo-20191028-2106/dojox/charting/scaler/primitive.js";
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0186 = "/maximo_mif/ui/maximo.jsp";
		String requestContent0186 = 
				"uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&csrftoken=" + C_CsrfToken2 +		// recorded value = 6q1fqknqkam6m2psbhf3v8tli2
				...
				"&requesttype=SYNC" +
				"&responsetype=text%2Fxml" +
				"&events=%5B%7B%22type%22%3A%22click%22%2C%22targetId%22%3A%22titlebar_hyperlink_8-lbsignout%22%2C%22value%22%3A%22%22%2C%22requestType%22%3A%22SYNC%22%2C%22csrftokenholder%22%3A%226q1fqknqkam6m2psbhf3v8tli2%22%7D%5D";
		requestContent0186 = Lib.replaceTextPattern(requestContent0186, "4152", C_UISessionID, 0);		// search and replace content fragment "4152" with variable 'C_UISessionID' on all occurrences
		requestContent0186 = Lib.replaceTextPattern(requestContent0186, "6q1fqknqkam6m2psbhf3v8tli2", C_CsrfToken2, 0);		// search and replace content fragment "6q1fqknqkam6m2psbhf3v8tli2" with variable 'C_CsrfToken2' on all occurrences

		byte[] requestContentBinary0186 = requestContent0186.getBytes("UTF-8");
		String requestHeader0186 = "POST " + requestFile0186 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Type: application/x-www-form-urlencoded;charset=UTF-8\r\n" + 
				"X-Requested-With: XMLHttpRequest\r\n" + 
				"Content-Length: " + requestContentBinary0186.length + "\r\n" + 		// Content-Length: 402
				"Origin: https://teck.maximo.com\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0186, requestHost0186, requestPort0186, requestHeader0186, requestContentBinary0186, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.replaceRequestHeaderFieldTextPattern("fbmcasdd7eo5cjv43dodp0473o", C_tt1);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("4152", C_UISessionID);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("672", C_PresentationID);		// search within all request header fields a text pattern and replace it with a variable value
		if (userNextProxyConfig != null)
		...
		requestContentBinary0186 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "TEXT/XML", header text fragment = [verification disabled], recorded content size = 236
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/XML", null, "![CDATA[https://teck.maximo.com/maximo");		// Test [186] <- Index 186
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		BoundaryBasedExtractorItem boundaryBasedContentExtractorItem0186_1 = new BoundaryBasedExtractorItem( "_tt=" , "]]>",1,false,false,0,false);
		boundaryBasedExtractor.addBoundaryBasedExtractorItem(boundaryBasedContentExtractorItem0186_1);
		...
		{
			// extract var 'C_tt2'
			C_tt2 = boundaryBasedContentExtractorItem0186_1.getResultText();
			log("<<< C_tt2 = " + C_tt2);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
		...
		String requestFile0187 = "/maximo_mif/webclient/login/logout.jsp" +
				"?uisessionid=" + C_UISessionID +		// recorded value = 4152
				"&_tt=" + C_tt2;		// recorded value = b2qm5brivv7kg6c3g83355436s
		requestFile0187 = Lib.replaceTextPattern(requestFile0187, "b2qm5brivv7kg6c3g83355436s", C_tt2, 0);		// search and replace request file fragment "b2qm5brivv7kg6c3g83355436s" with variable 'C_tt2' on all occurrences
		requestFile0187 = Lib.replaceTextPattern(requestFile0187, "4152", C_UISessionID, 0);		// search and replace request file fragment "4152" with variable 'C_UISessionID' on all occurrences
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 20
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [187] <- Index 187
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0188 = "/maximo_mif/webclient/login/login.jsp";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Enterprise Asset Management");		// Test [188] <- Index 188
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("StartCenterBuyerPOTabLT1", concurrentUsers, ParseArgs.getString(args, "-executionPlan"));
		...
		final int MAX_THREAD_STEPS = 185;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("StartCenterBuyerPOTabLT1");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new StartCenterBuyerPOTabLT1());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				StartCenterBuyerPOTabLT1 simulatedUser = new StartCenterBuyerPOTabLT1(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
